...
# ==============================================================================
# 1. CONFIGURAÇÕES DA PÁGININA E VARIÁVEIS GERAIS
# ==============================================================================
...
def classificar_sla(row):
    ...
    horario_corte_str = next((h for k, h in horarios_saida.items() if k in transp or transp in k), None)
    if not horario_corte_str: 
        return pd.Series([pd.NaT, 'Transportador Não Mapeado'])
        
    hora_corte = pd.to_datetime(horario_corte_str, format='%H:%M:%S').time()
    
    if dt_disp.weekday() == 5: 
        data_prazo = dt_disp.date() + timedelta(days=2)
    elif dt_disp.weekday() == 6: 
        data_prazo = dt_disp.date() + timedelta(days=1)
    elif dt_disp.time() > hora_corte:
        proximo_dia = dt_disp.date() + timedelta(days=1)
        data_prazo = proximo_dia + timedelta(days=2) if proximo_dia.weekday() == 5 else proximo_dia
    else: 
        data_prazo = dt_disp.date()
        
    prazo_limite = pd.to_datetime(f"{data_prazo} {horario_corte_str}")
...
if uploaded_file is not None:
    with st.spinner("Lendo e limpando os dados... Por favor, aguarde."):
        # Carregamento
    ...
    # Exibe métricas de limpeza na tela
    ...
    if not df_valido.empty:
        ...
        try:
            ...
            # Fallback caso a versão do Pandas seja mais antiga e não suporte include_groups
        ...
        # Filtra apenas as colunas que existem no CSV enviado para evitar erros
